...
test("sanitizes glob terminology from outbound prompt and tool descriptions", async () => {
  ...
  try {
      ...
      tools: [
        {
          ...
          parameters: { type: "object", properties: { pattern: { type: "string", description: "Glob pattern to match files" } } } as never,
        },
      ],
    };
    const logger = createTestLogger();
    ...
    const request = outboundBody as { params: { system?: string; tools?: Array<{ description?: string; input_schema?: unknown }> } };
...
test("sends native CommandCode stream request with tool schemas and emits text deltas", async () => {
  ...
  try {
    const context: Context = {
      ...
      messages: [{ role: "user", content: "Say hello", timestamp: Date.now() }],
      tools: [{ name: "read", description: "Read a file", parameters: { type: "object", properties: { path: { type: "string" } } } as never }],
    };
    const logger = createTestLogger();
    const stream = createCommandCodeStream(config, { cwd: process.cwd() }, logger)(model, context, { apiKey: "token" });
    const events = await collect(stream);
    ...
    assert.deepEqual(request.params.tools?.[0].input_schema, { type: "object", properties: { path: { type: "string" } } });
...
test("sends native CommandCode tools and still parses XML tool calls from fallback JSON", async () => {
  ...
  try {
    ...
    const request = outboundBody as { params: { tools?: Array<Record<string, unknown>>; messages: Array<{ content: string }> } };
    assert.equal(request.params.tools?.[0].name, "read");
...
test("serializes Pi tool results into CommandCode native tool-result content blocks", async () => {
  ...
  try {
    const context: Context = {
      ...
      messages: [
        { role: "user", content: "Read package.json", timestamp: Date.now() },
        {
          ...
          provider: "command-code",
          model: "test-model",
          content: [{ type: "toolCall", id: "command-code-tool-1", name: "read", arguments: { path: "package.json" } }],
    ...
    const request = outboundBody as { params: { messages: Array<{ role: string; content: Array<Record<string, unknown>> }> } };
...
test("does not inject fake reasoning instructions but still parses thinking blocks if upstream returns them", async () => {
  ...
  try {
    ...
    const logger = createTestLogger();
    const stream = createCommandCodeStream(config, { cwd: process.cwd() }, logger)(model, context, { apiKey: "token", reasoning: "high" });
    const events = await collect(stream);
