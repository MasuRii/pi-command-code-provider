...
function toolResultContent(message: Extract<Context["messages"][number], { role: "toolResult" }>): CommandCodeContentPart[] {
  return [
    {
      type: "tool-result",
...
function sanitizeOutboundValue(value: unknown): unknown {
  ...
  return Object.fromEntries(Object.entries(value).map(([key, entry]) => [sanitizeSchemaKey(key), sanitizeOutboundValue(entry)]));
}
...
function resolveApiKey(config: ExtensionConfig, options?: SimpleStreamOptions): string {
  ...
  if (!apiKey || (apiKey === config.apiKey && ENV_VAR_PATTERN.test(config.apiKey) && !process.env[config.apiKey])) {
    throw new Error(`No CommandCode API token configured. Set ${config.apiKey} or update pi-command-code-provider/config.json.`);
  }
  return apiKey;
}
...
function parseEventUsage(model: Model<Api>, rawUsage: unknown): Usage | undefined {
  ...
  const input = numberFrom(inputDetails.noCacheTokens ?? rawUsage.inputTokens) || Math.max(0, numberFrom(rawUsage.inputTokens) - numberFrom(rawUsage.cachedInputTokens));
...
function parseDsmlToolCalls(dsml: string, baseIndex: number): ToolCall[] {
  ...
  while ((invokeMatch = invokePattern.exec(dsml)) !== null) {
    ...
    const parameterPattern = /<｜｜DSML｜｜parameter\s+[^>]*name=["']([^"']+)["'][^>]*?(?:string=["']([^"']+)["'])?[^>]*>([\s\S]*?)<\/｜｜DSML｜｜parameter>/gi;
...
function parseTextMarkup(text: string, baseIndex: number): ParsedContentBlock[] {
  ...
  const pattern = /<thinking>([\s\S]*?)<\/thinking>|<tool_calls>([\s\S]*?)<\/tool_calls>|<｜｜DSML｜｜tool_calls>([\s\S]*?)<\/｜｜DSML｜｜tool_calls>/gi;
  let cursor = 0;
  ...
  while ((match = pattern.exec(text)) !== null) {
    ...
    if (match[1] !== undefined) {
      ...
      blocks.push(...parseXmlToolCalls(match[2], baseIndex + blocks.length).map((toolCall) => ({ type: "toolCall" as const, toolCall })));
    } else if (match[3] !== undefined) {
      blocks.push(...parseDsmlToolCalls(match[3], baseIndex + blocks.length).map((toolCall) => ({ type: "toolCall" as const, toolCall })));
    }
...
function emitTextDelta(stream: AssistantMessageEventStream, output: AssistantMessage, contentIndex: number, delta: string): void {
...
function emitThinkingDelta(stream: AssistantMessageEventStream, output: AssistantMessage, contentIndex: number, delta: string): void {
  const block = output.content[contentIndex];
...
function emitToolCall(stream: AssistantMessageEventStream, output: AssistantMessage, toolCall: ToolCall, context: Context): void {
  ...
  stream.push({ type: "toolcall_delta", contentIndex, delta: JSON.stringify(normalizedToolCall.arguments), partial: output });
  stream.push({ type: "toolcall_end", contentIndex, toolCall: normalizedToolCall, partial: output });
}

function emitResponse(stream: AssistantMessageEventStream, output: AssistantMessage, response: CommandCodeResponse, model: Model<Api>, context: Context): void {
  output.responseId = typeof response.id === "string" ? response.id : undefined;
  output.responseModel = typeof response.model === "string" && response.model !== model.id ? response.model : undefined;
...
async function consumeEventStream(response: Response, stream: AssistantMessageEventStream, output: AssistantMessage, model: Model<Api>, context: Context): Promise<void> {
  ...
  const handleEvent = (event: unknown): void => {
    ...
    if (type === "tool-call") {
      const id = optionalString(event.toolCallId) ?? optionalString(event.id) ?? `command-code-tool-${output.content.length}`;
...
async function executeCommandCodeRequest(
  ...
  try {
    ...
    logger.error("request_failed", { model: model.id, stopReason: output.stopReason, error: output.errorMessage });
