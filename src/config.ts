...
const OPENAI_REASONING_MAP = { off: "none", minimal: "minimal", low: "low", medium: "medium", high: "high", xhigh: null } satisfies ThinkingLevelMap;
...
const COMMAND_CODE_AUTO_REASONING_MAP = { off: "off", minimal: "minimal", low: "low", medium: "medium", high: "high", xhigh: null } satisfies ThinkingLevelMap;

const DEFAULT_RAW_MODELS: RawModel[] = [
  { id: "moonshotai/Kimi-K2.5", name: "Kimi K2.5", reasoning: false, contextWindow: 262144, maxTokens: 262144, cost: { input: 0.6, output: 3, cacheRead: 0.1, cacheWrite: 0 } },
  { id: "moonshotai/Kimi-K2.6", name: "Kimi K2.6", reasoning: false, contextWindow: 262144, maxTokens: 262144, cost: { input: 0.95, output: 4, cacheRead: 0.16, cacheWrite: 0 } },
  { id: "deepseek/deepseek-v4-pro", name: "DeepSeek V4 Pro", reasoning: true, thinkingLevelMap: COMMAND_CODE_AUTO_REASONING_MAP, contextWindow: 1048576, maxTokens: 393216, cost: { input: 0.435, output: 0.87, cacheRead: 0.003625, cacheWrite: 0 } },
  { id: "Qwen/Qwen3.6-Plus", name: "Qwen 3.6 Plus", reasoning: true, thinkingLevelMap: COMMAND_CODE_AUTO_REASONING_MAP, contextWindow: 1000000, maxTokens: 65536, cost: { input: 0.5, output: 3, cacheRead: 0.1, cacheWrite: 0 } },
  { id: "gpt-5.5", name: "GPT 5.5", reasoning: true, thinkingLevelMap: OPENAI_REASONING_MAP, contextWindow: 922000, maxTokens: 128000, cost: { input: 5, output: 30, cacheRead: 0.5, cacheWrite: 0 } },
  { id: "gpt-5.3-codex", name: "GPT 5.3 Codex", reasoning: true, thinkingLevelMap: OPENAI_REASONING_MAP, contextWindow: 272000, maxTokens: 128000, cost: { input: 2, output: 8, cacheRead: 0.5, cacheWrite: 0 } },
  { id: "claude-sonnet-4-6", name: "Claude Sonnet 4.6", reasoning: true, thinkingLevelMap: ANTHROPIC_REASONING_MAP, contextWindow: 1000000, maxTokens: 64000, cost: { input: 3, output: 15, cacheRead: 0.3, cacheWrite: 3.75 } },
  { id: "claude-opus-4-7", name: "Claude Opus 4.7", reasoning: true, thinkingLevelMap: ANTHROPIC_OPUS_4_7_REASONING_MAP, contextWindow: 1000000, maxTokens: 128000, cost: { input: 5, output: 25, cacheRead: 0.5, cacheWrite: 6.25 } },
  { id: "claude-opus-4-6", name: "Claude Opus 4.6", reasoning: true, thinkingLevelMap: ANTHROPIC_OPUS_4_6_REASONING_MAP, contextWindow: 1000000, maxTokens: 128000, cost: { input: 5, output: 25, cacheRead: 0.5, cacheWrite: 6.25 } },
  { id: "zai-org/GLM-5.1", name: "GLM 5.1", reasoning: false, contextWindow: 200000, maxTokens: 131072, cost: { input: 1.4, output: 4.4, cacheRead: 0.26, cacheWrite: 0 } },
  { id: "zai-org/GLM-5", name: "GLM 5", reasoning: false, contextWindow: 204800, maxTokens: 131072, cost: { input: 1, output: 3.2, cacheRead: 0.2, cacheWrite: 0 } },
];
...
function outputOr(value: unknown, fallback?: OutputModality[]): OutputModality[] | undefined {
  ...
  const parsed = value.filter(
    (entry): entry is OutputModality => entry === "text" || entry === "image" || entry === "audio" || entry === "video",
  );
...
function normalizeModel(rawModel: unknown, defaults: Omit<CommandCodeProviderModelConfig, "id" | "name">): CommandCodeProviderModelConfig | null {
  ...
  for (const key of ["baseUrl", "thinkingLevelMap", "output", "capabilities", "headers", "compat", "isFree", "importOwnership"] as const) {
...
function resolveCommandCodeVersion(raw: Record<string, unknown>, extensionRoot: string, warnings: string[]): string {
  ...
  if (configured?.toLowerCase() === COMMAND_CODE_VERSION_AUTO) {
    warnings.push(`commandCodeVersion is auto, but the CommandCode CLI package was not found; using fallback ${FALLBACK_COMMAND_CODE_VERSION}.`);
  }
  return FALLBACK_COMMAND_CODE_VERSION;
}

function readModelsJsonProvider(raw: Record<string, unknown>, extensionRoot: string, providerId: string, warnings: string[]): Record<string, unknown> | undefined {
  const configuredPath = optionalString(raw.modelsJsonPath);
  const modelsJsonPath = configuredPath ? resolveConfigPath(extensionRoot, configuredPath) : defaultModelsJsonPath(extensionRoot);
  ...
  try {
    ...
    warnings.push(`Unable to read models.json provider metadata: ${error instanceof Error ? error.message : "unknown error"}`);
    return undefined;
  }
}

function normalizeModelList(rawModels: unknown, defaults: Omit<CommandCodeProviderModelConfig, "id" | "name">): CommandCodeProviderModelConfig[] {
